...
export default AddressForm;
	...
	const options = SOs.map((sO) => ({id: sO.id, label: `${sO.description} - (${sO.price.raw}KZT)`}))
		useEffect(() => {
		if (SD) fetchShippingOptions(checkoutToken.id, SC, SD);
	}, [SD]);
	<Grid item xs = {12} sm = {6}>
							<InputLabel>Shipping Options</InputLabel>
							<Select value = {SO} fullWidth onChange = {(e) => setSO(e.target.value)} >
							{options.map((option) => (
